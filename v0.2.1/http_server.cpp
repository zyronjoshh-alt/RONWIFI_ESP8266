...
// ✅ FIX #4: Track window expiration for resume (set from main loop)
static uint32_t s_armWindowEndsMs = 0;

// ✅ FIX #3: Mutex to prevent auto-finalize / Done race
static volatile bool s_finalizeBusy = false;

// ✅ FIX #3: Cache last finalization result for late-arriving Done requests
...
// ✅ FIX #4: Called from main loop when coinslot enters ARMED state
void httpSetArmWindowEnds(uint32_t endsMs) {
  s_armWindowEndsMs = endsMs;
}
...
static void handleCoinStart() {
  ...
  if (cs == CS_ARMED || cs == CS_COUNTING || cs == CS_FINALIZING) {
    if (s_currentMac == mac) {
      ...
      // ✅ FIX #4: Calculate remaining seconds based on tracked window end
      uint32_t remaining_secs = (s_armWindowEndsMs > millis()) 
                               ? (s_armWindowEndsMs - millis()) / 1000 
                               : 0;
      String body = String("{\"session_id\":\"") + s_currentSessionId + "\",\"state\":\"" + 
                    (cs == CS_ARMED ? "armed" : "counting") + 
                    "\",\"window_seconds\":" + remaining_secs + "}";  // ✅ FIX #4: Return remaining, not full
      sendJsonOk(body);
      return;
    } else {
  ...
  // 3. START NEW SESSION
  // ✅ FIX: Removed MikroTik pre-check - we'll check when creating user in /coin/done
  // This eliminates 1.5s blocking delay and "Coinslot Unavailable" errors on first try
...
static void handleCoinDone() {
  ...
  String requestMac = doc["mac"] | "";
  requestMac.toUpperCase();

  // ✅ FIX #3: Check if auto-finalize is currently running
  if (s_finalizeBusy) {
    sendJsonError(503, "busy", "Finalization in progress, retry in 1 second");
    return;
  }

  // ✅ FIX #3: Check cache for late-arriving Done after auto-finalize completed
  if (coinslotGetState() == CS_IDLE) {
    if (s_finalizeCache.valid && 
        s_finalizeCache.mac == requestMac && 
        (millis() - s_finalizeCache.timestamp_ms) < 10000) {
      // Auto-finalize just handled this MAC <10s ago, return cached success
      String body = String("{\"pesos\":") + s_finalizeCache.pesos +
                    ",\"minutes\":" + s_finalizeCache.minutes +
                    ",\"username\":\"" + macWithColons(requestMac) + "\"" +
                    ",\"password\":\"" + macWithColons(requestMac) + "\"" +
                    ",\"status\":\"ok\"}";
      sendJsonOk(body);
      Serial.println(F("[done] Served cached result from auto-finalize"));
      return;
    }
...
if (pesos > 0) {
  ...
  if (pesos == 0) {
    coinslotDisarm();
    s_finalizeBusy = false;  // ✅ FIX #3: Unlock
    sendJsonOk("{\"pesos\":0,\"minutes\":0,\"status\":\"cancelled\"}");
    s_currentSessionId = "";
    s_currentMac = "";  // ✅ Clear stale MAC
    s_currentIp = "";
    return;
  }
  ...
  // ✅ FIX #3: Cache this result before unlocking
  s_finalizeCache.mac = s_currentMac;
  s_finalizeCache.pesos = pesos;
  s_finalizeCache.minutes = minutes;
  s_finalizeCache.timestamp_ms = millis();
  s_finalizeCache.valid = true;

  String body = String("{\"pesos\":") + pesos +
                ",\"minutes\":" + minutes +
                ",\"username\":\"" + macWithColons(s_currentMac) + "\"" +
                ",\"password\":\"" + macWithColons(s_currentMac) + "\"" +
                ",\"status\":\"ok\"}";
  sendJsonOk(body);
...
static void handleCoinCancel() {
  ...
  // Note: Strike is NOT removed on cancel (counts as abuse)
  // This prevents spam: repeated insert-cancel attempts
...
void httpAutoFinalizeSession() {
  ...
  // ✅ FIX #3: Skip if user's Done is currently handling it
  if (s_finalizeBusy) {
    Serial.println(F("[auto] Skipping (Done in progress)"));
    return;
  }
  ...
  // ✅ FIX #3: Cache this result for late-arriving Done requests
  s_finalizeCache.mac = s_currentMac;
  s_finalizeCache.pesos = pesos;
  s_finalizeCache.minutes = minutes;
  s_finalizeCache.timestamp_ms = millis();
