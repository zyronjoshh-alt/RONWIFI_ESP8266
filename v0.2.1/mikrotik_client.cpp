...
bool mikrotikCreateAndLoginUser(const String& macNoColons,
  ...
  if (exists) {
    // ✅ FIX #6: Extend existing user (ADDITIVE limit-uptime)
    // Parse current limit-uptime from response (e.g., "limit-uptime":"02:30:00" or "2d 3h")
    int limitIdx = resp.indexOf("\"limit-uptime\"");
    uint32_t currentMinutes = 0;
    
    if (limitIdx >= 0) {
      // Extract the value between quotes after "limit-uptime":"
      int start = resp.indexOf('"', limitIdx + 16); // skip past "limit-uptime":
      int end = resp.indexOf('"', start + 1);
      if (start >= 0 && end > start) {
        String limitStr = resp.substring(start + 1, end);
        // Parse HH:MM:SS or Nd Hh formats
        // Simple parser: count days, hours, mins
        int dIdx = limitStr.indexOf('d');
        int hIdx = limitStr.indexOf('h');
        int colIdx = limitStr.indexOf(':');
        
        if (dIdx > 0) {
          // Format: "Nd Hh" or "Nd"
          uint32_t d = limitStr.substring(0, dIdx).toInt();
          currentMinutes = d * 1440;
          if (hIdx > dIdx) {
            uint32_t h = limitStr.substring(dIdx + 2, hIdx).toInt();
            currentMinutes += h * 60;
          }
        } else if (colIdx > 0) {
          // Format: "HH:MM:SS"
          uint32_t h = limitStr.substring(0, colIdx).toInt();
          uint32_t m = limitStr.substring(colIdx + 1, limitStr.indexOf(':', colIdx + 1)).toInt();
          currentMinutes = h * 60 + m;
        }
      }
    }
    
    uint32_t totalMinutes = currentMinutes + minutes;
    String newDur = formatDuration(totalMinutes);
    
    Serial.print(F("[mikrotik] extend: old="));
